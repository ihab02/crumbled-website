-- Migration: Add Enhanced Promo Codes and Pricing Management System
-- Date: 2025-01-27
-- Description: Comprehensive migration for both enhanced promo codes and pricing management

-- =====================================================
-- ENHANCED PROMO CODES SYSTEM
-- =====================================================

-- Enhance existing promo_codes table with new fields
ALTER TABLE promo_codes 
ADD COLUMN enhanced_type ENUM('basic', 'free_delivery', 'buy_one_get_one', 'buy_x_get_y', 'category_specific', 'first_time_customer', 'loyalty_reward') DEFAULT 'basic' AFTER discount_type,
...
ADD COLUMN minimum_quantity INT NULL AFTER first_time_only,
ADD COLUMN maximum_quantity INT NULL AFTER minimum_quantity,
ADD COLUMN combination_allowed BOOLEAN DEFAULT TRUE AFTER maximum_quantity,
ADD COLUMN stack_with_pricing_rules BOOLEAN DEFAULT TRUE AFTER combination_allowed,
ADD COLUMN buy_x_quantity INT NULL AFTER stack_with_pricing_rules,
ADD COLUMN get_y_quantity INT NULL AFTER buy_x_quantity,
ADD COLUMN get_y_discount_percentage DECIMAL(5,2) NULL AFTER get_y_quantity,
...
-- Update products table with pricing fields
ALTER TABLE products 
ADD COLUMN original_price DECIMAL(10, 2) NULL AFTER base_price,
ADD COLUMN sale_price DECIMAL(10, 2) NULL AFTER original_price,
ADD COLUMN sale_start_date DATETIME NULL AFTER sale_price,
ADD COLUMN sale_end_date DATETIME NULL AFTER sale_start_date,
ADD COLUMN is_on_sale BOOLEAN DEFAULT FALSE AFTER sale_end_date,
ADD INDEX idx_is_on_sale (is_on_sale),
ADD INDEX idx_sale_start_date (sale_start_date),
ADD INDEX idx_sale_end_date (sale_end_date),
ADD INDEX idx_sale_active (is_on_sale, sale_start_date, sale_end_date);

-- Update orders table to track both pricing rules and promo codes
ALTER TABLE orders 
ADD COLUMN applied_pricing_rules JSON NULL AFTER promo_code,
ADD COLUMN pricing_discount_amount DECIMAL(10, 2) DEFAULT 0.00 AFTER discount_amount,
ADD COLUMN total_discount_amount DECIMAL(10, 2) GENERATED ALWAYS AS (discount_amount + pricing_discount_amount) STORED;
...
-- Update existing promo codes to enhanced type
...
INSERT INTO pricing_categories (name, description, category_type, target_value) VALUES 
...
INSERT INTO product_prices (product_id, price_type, price, is_active, created_by) VALUES 
