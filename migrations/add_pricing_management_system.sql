-- Migration: Add Pricing Management System
-- Date: 2025-01-27
-- Description: Add pricing management system tables and fields
...
ADD COLUMN IF NOT EXISTS original_price DECIMAL(10, 2) NULL AFTER base_price,
ADD COLUMN IF NOT EXISTS sale_price DECIMAL(10, 2) NULL AFTER original_price,
...
ADD COLUMN IF NOT EXISTS is_on_sale BOOLEAN DEFAULT FALSE AFTER sale_end_date;

-- Add indexes for products table (only if they don't exist)
CREATE INDEX IF NOT EXISTS idx_is_on_sale ON products(is_on_sale);
CREATE INDEX IF NOT EXISTS idx_sale_start_date ON products(sale_start_date);
CREATE INDEX IF NOT EXISTS idx_sale_end_date ON products(sale_end_date);
CREATE INDEX IF NOT EXISTS idx_sale_active ON products(is_on_sale, sale_start_date, sale_end_date);

-- Update orders table to track both pricing rules and promo codes
ALTER TABLE orders 
ADD COLUMN IF NOT EXISTS applied_pricing_rules JSON NULL AFTER promo_code,
ADD COLUMN IF NOT EXISTS pricing_discount_amount DECIMAL(10, 2) DEFAULT 0.00 AFTER discount_amount;

-- Add total_discount_amount as generated column if it doesn't exist
SET @sql = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
   WHERE TABLE_SCHEMA = 'crumbled_nextDB' 
   AND TABLE_NAME = 'orders' 
   AND COLUMN_NAME = 'total_discount_amount') = 0,
  ...
  'SELECT "total_discount_amount column already exists"'
));
